...
export default function Controls() {
  ...
  const startScreenSharing = async () => {
    try {
      const screenSharingTrack = await AgoraRTC.createScreenVideoTrack({ encoderConfig: '1080p_1' });
  ...
  const stopScreenSharing = async () => {
    if (screenTrack) {
      ...
      if (tracks && tracks[1]) await client.publish(tracks[1]); // restore camera
  ...
  useEffect(() => {
    if (screenSharing && screenTrack && tracks && tracks[1]) {
      const fun = async () => {
